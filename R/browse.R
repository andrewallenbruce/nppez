...
#' @autoglobal
#' @export
ask <- function(save = FALSE, dir = NULL) {
  ...
  "(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+(\\d{1,2})\\,\\s+(\\d{4})"),
      ...
      links = NULL, full = NULL) |>
    tidyr::unite("release_date", date1:date2,
                 remove = TRUE, sep = "", na.rm = TRUE) |>
    dplyr::mutate(release_date = clock::date_parse(release_date),
                  current_date = clock::date_today("")) |>
    dplyr::mutate(file_age = clock::date_count_between(
      release_date, current_date, "day"), current_date = NULL) |>
    dplyr::relocate(file_age, .after = release_date)

  if (save) {
    readr::write_csv(results, file = paste0(dir,
          "nppes_data_dissemination_info_", results[[1]][[1]], ".csv"))
  }
...
peek <- function(dir) {
  ...
  fs::dir_info(dir) |>
    dplyr::select(path) |>
    dplyr::mutate(zip = stringr::str_ends(path, ".zip")) |>
    dplyr::filter(zip == TRUE) |>
    dplyr::mutate(zip = NULL) |>
    tibble::deframe() |>
...
prune <- function(dir) {
  ...
  fs::dir_info(dir) |>
    ...
    purrr::map(zip::zip_list) |>
    purrr::list_rbind(names_to = "parent_zip") |>
    dplyr::mutate(contains_month = stringr::str_detect(parent_zip,
    "(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)")) |>
    dplyr::filter(contains_month == TRUE) |>
...
dispense <- function(zip_dir,
  ...
  fs::dir_info(zip_dir) |>
    dplyr::select(path) |>
    tibble::deframe() |>
    purrr::walk(zip::unzip, exdir = unzip_dir, files = files)
